...
var convertDataFormat = function(record) {
  ...
  var initialPart = record.split(' ', 5)
  if (initialPart.length < 5) {
    hangover = record
    throw new Error('Initial part not 5 records long! ' + initialPart.length)
  }
  transformed.host = initialPart[0]
  transformed.ip = initialPart[1]
  transformed.date = new Date(Date.parse(initialPart[4]))
  transformed.rawTimestamp = initialPart[4]
  ...
  transformed.referrer = nextPart[4]
  transformed.userAgent = nextPart[5]
  transformed.bot = nextPart[6]
  nextPart = null
  initialPart = null
  return transformed
}
